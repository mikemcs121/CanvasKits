...
Use `info/assets/` for canonical images, stages and logo variants; `info/tmp/` for editable plans, masks, prompts, builders and reviews; `info/output/pdf/8x10/` for matching PNGs and compatibility PDFs; and `info/archive/` only for supplied originals (earlier editions live in git history; commit before replacing files). Keep an `info/README.md` pointing to the active source and builder. Preserve prior dependency trees when moving files, and record a path/hash manifest. Treat project-root production indexes as the entrypoint instead of leaving extra notes in each kit root.

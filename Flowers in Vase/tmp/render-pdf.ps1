...
$pdfToRender = if ($env:GNOME_PDF_PATH) { $env:GNOME_PDF_PATH } else { 'output/pdf/gnome-painting-guide.pdf' }
...
for ($i=0; $i -lt $doc.PageCount; $i++) {
  ...
  $outputStream = [System.IO.File]::Create((Join-Path (Get-Location) ('tmp/page-' + ($i+1) + '.png')))

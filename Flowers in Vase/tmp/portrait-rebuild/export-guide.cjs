...
const png = fs.readFileSync(path.join(kit, 'tmp/portrait-rebuild/guide-page.png'));
...
const destination=path.join(__dirname,'flowers-in-vase-painting-guide-8x10.pdf');fs.writeFileSync(destination,Buffer.concat(parts));

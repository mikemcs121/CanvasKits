...
const {bmp}=require('../../../Fox Fall/tmp/raster.cjs');
...
for(const [folder,slug] of kits)for(const kind of ['outline','finished-reference']){
 const name=`${slug}-${kind}-8x10`,src=path.resolve(folder,'tmp/8x10',name+'.bmp'),out=path.resolve(folder,'tmp/8x10',name+'.pdf');

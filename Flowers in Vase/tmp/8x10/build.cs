...
static Bitmap Edition(Bitmap src){var b=new Bitmap(2400,3000,PixelFormat.Format24bppRgb);b.SetResolution(300,300);using(var g=Graphics.FromImage(b)){g.Clear(Color.White);g.InterpolationMode=InterpolationMode.HighQualityBicubic;g.DrawImage(src,60,360,2280,2280);}return b;}
static void Save(Bitmap b,string name){b.Save(root+"/assets/8x10/"+name+".png",ImageFormat.Png);}
public static void Run(string r){root=r;outline=new Bitmap(root+"/flowers-in-vase-outline.png");paint=new Bitmap(root+"/assets/8x10/flowers-in-vase-registered-reference.png");w=outline.Width;h=outline.Height;petalMask=File.ReadAllBytes(root+"/tmp/petal-blend-mask.bin");group=new int[300];using(var br=new BinaryReader(File.OpenRead(root+"/tmp/regions.bin"))){br.ReadInt32();br.ReadInt32();ids=new int[w*h];for(int i=0;i<ids.Length;i++)ids[i]=br.ReadInt32();}
...
var stages=new Bitmap[12]; for(int s=1;s<=11;s++){string sp=root+"/assets/8x10/stage-"+s.ToString("00")+".png";if(File.Exists(sp)&&File.GetLastWriteTimeUtc(sp)>File.GetLastWriteTimeUtc(root+"/assets/8x10/flowers-in-vase-registered-reference.png")){stages[s]=new Bitmap(sp);}else{stages[s]=Stage(s);if(s>=11)foreach(int id in berries)for(int y=0;y<h;y++)for(int x=0;x<w;x++)if(ids[y*w+x]==id)stages[s].SetPixel(x,y,paint.GetPixel(x,y));Save(stages[s],"stage-"+s.ToString("00"));}}
foreach(var item in new[]{"outline","finished-reference"}){var eb=Edition(item=="outline"?outline:paint);string name="flowers-in-vase-"+item+"-8x10";eb.Save(root+"/tmp/8x10/"+name+".png",ImageFormat.Png);eb.Save(root+"/tmp/8x10/"+name+".bmp",ImageFormat.Bmp);eb.Dispose();}
var panels=new Bitmap[12];for(int i=0;i<8;i++)panels[i]=Edition(stages[i+1]);panels[8]=Crop(stages[9],new Rectangle(595,390,425,365));panels[9]=Crop(stages[10],new Rectangle(0,570,790,680));panels[10]=Crop(stages[11],new Rectangle(935,655,315,420));
for(int i=8;i<11;i++){var cg=Graphics.FromImage(panels[i]);cg.SmoothingMode=SmoothingMode.AntiAlias; if(i==8)Cue(cg,180,130,100,100);if(i==9)Cue(cg,380,300,230,170);if(i==10)Cue(cg,167,209,105,90);cg.Dispose();Save(panels[i],"panel-"+(i+1));}
var edition=Edition(paint); var dry=new Bitmap(600,500);using(var dg=Graphics.FromImage(dry)){dg.Clear(Color.FromArgb(235,231,222));dg.SmoothingMode=SmoothingMode.AntiAlias;dg.FillPolygon(B(Color.FromArgb(198,194,184)),new PointF[]{new PointF(90,47),new PointF(374,70),new PointF(347,434),new PointF(62,412)});dg.DrawImage(edition,new PointF[]{new PointF(94,30),new PointF(376,53),new PointF(66,406)});for(int i=0;i<4;i++){dg.FillEllipse(B(new[]{teal,navy,Color.FromArgb(217,150,51),Color.FromArgb(176,63,92)}[i]),460,80+i*72,52,52);dg.DrawEllipse(new Pen(Color.White,4),460,80+i*72,52,52);dg.DrawEllipse(new Pen(Color.FromArgb(110,110,100),2),465,85+i*72,42,42);}Cue(dg,462,381,92,50);Cue(dg,450,414,92,50);} panels[11]=dry;Save(dry,"panel-12");
...
File.WriteAllLines(root+"/tmp/8x10/captions.txt",captions);

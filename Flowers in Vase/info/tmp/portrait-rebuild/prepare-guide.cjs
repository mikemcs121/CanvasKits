...
static string ProjectLogo(string r){var d=new DirectoryInfo(r);var project=d.Name.Equals("info",StringComparison.OrdinalIgnoreCase)?d.Parent.Parent:d.Parent;return Path.Combine(project.FullName,"assets","images","river-and-ridge-logo.png");}
...
`;
const guideSource=(prefix+layout+'Console.WriteLine("Portrait guide built with existing Flowers layout; captions fit.");\n}\n}\n').replace('Path.GetFullPath(root+"/../assets/images/river-and-ridge-logo.png")','ProjectLogo(root)');
fs.writeFileSync(path.join(__dirname,'build-guide.cs'),guideSource);
...
let render=fs.readFileSync(path.join(kit,'tmp/8x10/render.ps1'),'utf8').replaceAll('/tmp/8x10','/tmp/portrait-rebuild');
render=render.replace("$d=Join-Path (Get-Location) ($folder+'/tmp/portrait-rebuild');$out=Join-Path (Get-Location) ($folder+'/tmp/portrait-rebuild')","$d=$PSScriptRoot;$out=$PSScriptRoot");

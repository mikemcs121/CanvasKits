...
const functions=source.slice(source.indexOf('function vectorPdf'),source.indexOf('(async()=>')).replace('const scale=2280/1254*.24','const scale=576/1120').replace('14.4 633.6 cm','0 720 cm');
...
(async()=>{
 ...
 const paint=await Jimp.read(path.join(kit,'assets/portrait-rebuild/finished-reference-canonical.png'));paint.resize(2400,3000,Jimp.RESIZE_BICUBIC);fs.writeFileSync(path.join(__dirname,slug+'-finished-reference-8x10.png'),context.pngDpi(await paint.getBufferAsync(Jimp.MIME_PNG),300));imagePdf(path.join(__dirname,slug+'-finished-reference-8x10.pdf'),paint);

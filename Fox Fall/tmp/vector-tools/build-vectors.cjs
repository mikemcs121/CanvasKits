...
const potrace=require('potrace'),Jimp=require('jimp'),{Resvg}=require('@resvg/resvg-js');
...
(async()=>{
 ...
 for(const [index,[folder,slug]] of kits.entries()){
  ...
  const source=path.join(kit,'tmp/8x10-artwork/canonical-outline.png'),raw=await trace(source),tag=raw.match(/<path\b[^>]+\/>/)[0],d=tag.match(/\bd="([^"]+)"/)[1];
  ...
  fs.writeFileSync(path.join(tmp,slug+'-outline-8x10.svg'),svg);vectorPdf(path.join(tmp,slug+'-outline-8x10-vector.pdf'),d);
  ...
  // Thin line masks are sensitive to subpixel antialiasing. Reject changed interiors or
  // dropped features separately from expected boundary smoothing at native resolution.
  const iou=intersection/union;if(iou<.96||farDifferences>10)throw Error(`Trace mismatch ${folder}: IoU ${iou}; far differences ${farDifferences}`);

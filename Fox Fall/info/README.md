...
- `archive/previous-root-files/`: supplied originals (`download.png`).
- `output/pdf/8x10/fox-fall-finished-reference-8x10.png`: the reference master PNG (2400 x 3000). The reference PDF, the guide's preview and its drying panel all come from it.
...
- `tmp/build-new-panels.cjs`, `raster.cjs`, `kit-paths.cjs`, `new-kits.cjs`: the stage-panel builder for all four autumn kits. Step order, paint groups and crops are in `new-kits.cjs`.
...
- **Guide text or steps:** edit this kit's entry in `assets/tools/guide-template/kits.json`, then run `node assets/tools/guide-template/build.cjs fox-fall`. The new PDF is written to `tmp/template-refactor/`. Check it with `assets/tools/pdf-render/render-pdf.cjs` and copy it to the kit root.
...
- **Reference:** edit the master PNG, then run `node assets/tools/reference-pdf/png-to-pdf.cjs "Fox Fall/info/output/pdf/8x10/fox-fall-finished-reference-8x10.png" "Fox Fall/fox-fall-finished-reference-8x10.pdf"` and rebuild the guide.
...
- **Kit-root assets/:** after a guide change, empty `../assets/` and run `node Tools/guide-image-extractor.cjs "Fox Fall"`.
- **Simplified edition:** `python "Fox Fall/info/tmp/simplified/build.py"` (requires Python with reportlab), then `promote.py` in the same folder copies the result into `../simplified/`.

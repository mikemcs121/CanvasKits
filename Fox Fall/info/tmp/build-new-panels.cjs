...
const {workPath}=require('./kit-paths.cjs');
...
for(const k of kits){
 const d=workPath(k,'tmp','8x10-artwork'),o=bmp(d+'/outline.bmp'),p=bmp(d+'/paint.bmp'),{w,h}=o,N=w*h,s=segment(o),names=['white',...Object.keys(k.groups)],rid={};
 ...
 for(let i=0;i<N;i++){
  ...
  if(k.slug==='cat-in-pumpkin'&&s.labels[i]===2&&y<300&&r>145&&r>v*1.25&&v>b*.95)groups[i]=names.indexOf('pink');

...
base_stage = Image.open(KIT / "info/tmp/8x10-artwork/step-7.png").convert("RGB")
source = Image.open(ASSETS / f"{SLUG}-final-step-7-black-paws.png").convert("RGB")
base_array = np.array(base_stage)
source_array = np.array(source)
groups = np.frombuffer(
    (KIT / "info/tmp/8x10-artwork/group-mask.bin").read_bytes(), dtype=np.uint8
).reshape(source_array.shape[:2])
paws_mask = groups == 7
changed_mask = np.any(source_array != base_array, axis=2)
assert np.array_equal(changed_mask, paws_mask)
assert np.all(source_array[paws_mask] == 0)
assert np.array_equal(source_array[~paws_mask], base_array[~paws_mask])
...
assert embedded_exact, "Reference PDF image differs from the black-paws target"
...
assert "dark brown" not in guide_text_lower
...
checks = {
    "referenceEmbeddedPixelsExactlyBlackPawsTarget": embedded_exact,
    "blackPawsNativeMaskPixels": int(paws_mask.sum()),
    "blackPawsChangedPixels": int(changed_mask.sum()),
    "allChangedPixelsExactlyNativePawsMask": True,
    "allPawsMaskPixelsBlack": True,
    "allPixelsOutsidePawsMaskUnchangedFromOriginalStep7": True,
    ...
    "providedPaintsPresentAndDarkBrownAbsent": True,
    "originalProductionHashesUnchanged": True,
    "outlineTrueVectorNoBackground": True,
    "outlineInches": [8, 10],
    "outlineColor": "#808080",
    "outlineStrokePt": 2,
    "note": "The native paws group mask changes only the former dark-brown paws and ear tips to black; every other stage-7 pixel is unchanged. Texture and highlight stages 8-11 are omitted. The established square artwork remains centered without stretching. Existing teaching linework is retained; the approved transfer remains unchanged.",
}

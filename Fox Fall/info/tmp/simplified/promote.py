...
DEST = KIT / "simplified"
ARCHIVE = KIT / "info/archive/simplified-before-black-paws-2026-09-23/simplified"
SLUG = "fox-fall"
...
required = [
    "referenceEmbeddedPixelsExactlyBlackPawsTarget",
    "allChangedPixelsExactlyNativePawsMask",
    "allPawsMaskPixelsBlack",
    "allPixelsOutsidePawsMaskUnchangedFromOriginalStep7",
    "mixingLanguageAbsent",
    "providedPaintsPresentAndDarkBrownAbsent",
...
for name in names:
    ...
    if destination.exists() and sha256(destination) != sha256(source):
        ...
        if not archived.exists() or sha256(archived) != sha256(destination):
            raise RuntimeError(
                "Existing alternate is not preserved in the revision archive: "
                + str(destination)
            )
...
promotion = {
    ...
    "review": "Final PDF renders and the outline-registration contact sheet passed visual review. Paws and ear tips are black; dark brown is removed from the guide.",

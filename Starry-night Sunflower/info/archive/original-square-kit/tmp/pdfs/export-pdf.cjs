...
const png = fs.readFileSync(path.join(kit, 'assets/starry-night-sunflower-guide-artwork-draft.png'));
...
const scale=Math.min(576/width,756/height),w=width*scale,h=height*scale,x=(612-w)/2,y=(792-h)/2;
...
const objects=[Buffer.from('<< /Type /Catalog /Pages 2 0 R >>'),Buffer.from('<< /Type /Pages /Kids [3 0 R] /Count 1 >>'),Buffer.from('<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /XObject << /Im0 4 0 R >> >> /Contents 5 0 R >>'),stream(`/Type /XObject /Subtype /Image /Width ${width} /Height ${height} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /FlateDecode`,imageData),stream('',content),Buffer.from('<< /Title (Starry Night Sunflower Painting Guide - DRAFT) /Author (River and Ridge) >>')];
...
const destination=path.join(__dirname,'starry-night-sunflower-painting-guide-illustrated-draft.pdf');fs.writeFileSync(destination,Buffer.concat(parts));

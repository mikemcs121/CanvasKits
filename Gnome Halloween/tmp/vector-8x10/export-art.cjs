...
(async()=>{
 ...
 const paint=await Jimp.read(path.join(kit,'assets/8x10/finished-reference-canonical.png'));paint.resize(2400,3000,Jimp.RESIZE_BICUBIC);fs.writeFileSync(path.join(__dirname,slug+'-finished-reference-8x10.png'),context.pngDpi(await paint.getBufferAsync(Jimp.MIME_PNG),300));imagePdf(path.join(__dirname,slug+'-finished-reference-8x10.pdf'),paint);

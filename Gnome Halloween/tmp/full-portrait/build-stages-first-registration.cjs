...
function good(r,g,b,k){switch(k){case 1:return b>r*1.08&&g>r*.95&&b>g*.94;case 2:return r>g*1.05&&g>b*1.05;case 3:return r>160&&g>140&&b>65&&r>=g*.94&&g>b*.99;case 4:case 12:return Math.max(r,g,b)<100&&r>b*.62;case 5:return r>g*1.15&&b>g*.9&&r>b*.7;case 6:return r>g*1.12&&g>b*1.1;case 7:return r>160&&g>95&&b>50&&r>g*1.1&&b>g*.35;case 8:return r>g*1.15&&g>b*1.1&&r<225;case 9:return r>125&&g>120&&b>100&&r>=g*.96&&b<g*1.12;case 10:return r>50&&g>40&&g>b*1.3&&r<g*1.75;case 11:return r>145&&r>g*.99&&g>b*1.2;}return false;}
(async()=>{
 const line=await Jimp.read(path.join(asset,'outline-canonical-white.png')),paint=await Jimp.read(path.join(asset,'painting-texture-repaired.png')).then(p=>p.resize(w,h)),raw=paint.bitmap.data,bin=fs.readFileSync(path.join(__dirname,'regions.bin')),ids=new Int32Array(n),groups=new Uint8Array(n),rgb=Buffer.alloc(n*3),samples=Buffer.alloc(n*3),done=new Uint8Array(n),repair=new Uint8Array(n),q=new Int32Array(n);
 ...
 let head=0,tail=0;for(let i=0;i<n;i++){const x=i%w,y=(i/w)|0,k=groups[i];let sx=x,sy=y*.96;if(k===3){sx=mx0+(x-75)/270*(mx1-mx0);sy=my0+(y-65)/354*(my1-my0);} if(ids[i]===4){sx=848+(x-864)/285*312;sy=141+(y-159)/157*180;} if(ids[i]===5){sx=913+(x-943)/192*231;sy=404+(y-424)/106*144;}sx=Math.max(0,Math.min(w-1,Math.round(sx)));sy=Math.max(0,Math.min(h-1,Math.round(sy)));let p=(sy*w+sx)*4;let r=raw[p],g=raw[p+1],b=raw[p+2];
 ...
 samples[i*3]=r;samples[i*3+1]=g;samples[i*3+2]=b;if(k&&good(r,g,b,k)){done[i]=1;q[tail++]=i;rgb[i*3]=r;rgb[i*3+1]=g;rgb[i*3+2]=b;}else repair[i]=1;}
 ...
 const reference=new Jimp(w,h,0xffffffff);for(let i=0;i<n;i++)for(let c=0;c<3;c++)reference.bitmap.data[i*4+c]=rgb[i*3+c];await reference.writeAsync(path.join(asset,'finished-reference-canonical.png'));await reference.clone().resize(720,900).writeAsync(path.join(__dirname,'registered-reference-review.png'));

...
foreach($folder in @('fall view','Starry-night Sunflower')){
 if(Test-Path -LiteralPath ($folder+'/info/tmp/organization-repair')){$folder+='/info'};$d=Join-Path (Get-Location) ($folder+'/tmp/organization-repair');$out=Join-Path (Get-Location) ($folder+'/tmp/organization-repair');$checks=@()

...
- `archive/previous-root-files/`: supplied originals (`gnome-christmas-tree-finished-reference.png`).
...
- **Guide text or steps:** edit this kit's entry in `assets/tools/guide-template/kits.json`, then run `node assets/tools/guide-template/build.cjs gnome-christmas-tree`. The new PDF is written to `tmp/reference-restoration/`. Check it with `assets/tools/pdf-render/render-pdf.cjs` and copy it to the kit root.
- **Step pictures:** `node assets/tools/reference-restoration/guide-inputs.cjs` rebuilds `guide-step-NN.png` from `step-NN.png`. `build-art.cjs gnome-christmas-tree` regenerates the master and stages from the candidate; it overwrites `restored-master.png`, so run it only for a full repaint.
- **Reference:** edit the master PNG, then run `node assets/tools/reference-pdf/png-to-pdf.cjs "Gnome Christmas Tree/info/assets/reference-restoration/restored-master.png" "Gnome Christmas Tree/gnome-christmas-tree-finished-reference-8x10.pdf"` and rebuild the guide.
...
- **Kit-root assets/:** after a guide change, empty `../assets/` and run `node Tools/guide-image-extractor.cjs "Gnome Christmas Tree"`.

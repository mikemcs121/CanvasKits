...
foreach($k in $kits){
 if($KitFolder -and $k[0] -ne $KitFolder){continue}
 $root=Join-Path (Get-Location) $k[0];$dir=Get-KitWorkRoot $k[0];$tmp=Join-Path $dir 'tmp/8x10-artwork';$out=Join-Path $dir 'output/pdf/8x10';$base=$k[1]+'-painting-guide-8x10'
 ...
 Copy-Item -LiteralPath (Join-Path $tmp ($base+'.pdf')) -Destination (Join-Path $root ($base+'.pdf'))
 ...
 Write-Output ($k[0]+': guide PDF promoted to production root; working PDF/PNG verified and saved under the working output tree')
}

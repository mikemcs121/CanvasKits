...
- `archive/previous-root-files/`: supplied originals (`exec-dd3ad511-2377-40d8-bb15-dd9d9771fcbc.png`).
- `output/pdf/8x10/pumpkin-finished-reference-8x10.png`: the reference master PNG (2400 x 3000). The reference PDF, the guide's preview and its drying panel all come from it.
...
- **Guide text or steps:** edit this kit's entry in `assets/tools/guide-template/kits.json`, then run `node assets/tools/guide-template/build.cjs pumpkin`. The new PDF is written to `tmp/template-refactor/`. Check it with `assets/tools/pdf-render/render-pdf.cjs` and copy it to the kit root.
...
- **Reference:** edit the master PNG, then run `node assets/tools/reference-pdf/png-to-pdf.cjs "Pumpkin/info/output/pdf/8x10/pumpkin-finished-reference-8x10.png" "Pumpkin/pumpkin-finished-reference-8x10.pdf"` and rebuild the guide.
...
- **Kit-root assets/:** after a guide change, empty `../assets/` and run `node Tools/guide-image-extractor.cjs "Pumpkin"`.
- **Simplified edition:** `python "Pumpkin/info/tmp/simplified/build.py"` (requires Python with reportlab), writes the new PDFs to `tmp/simplified/`. Copy them into `../simplified/`, then run `verify.py` in the same folder to check them.

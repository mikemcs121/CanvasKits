...
Keep matching PNGs, compatibility PDFs, canonical images, masks, prompts, builders and review notes inside info/. Use info/assets/ for working artwork, info/tmp/ for builds and reviews, info/output/ for supporting exports, and info/test/ for retained supplied test art. Keep original supplied material unchanged.

September 25, 2026 cleanup (by the user's request): each `info/` folder now holds only what current tools read, the sources needed to rebuild guide steps, simplified editions and restored references, current checks and records, and supplied originals (`archive/previous-root-files/` source images, `archive/original-square-kit/`, `assets/original-square/`, `assets/reference-restoration/sources/`, `test/`). Superseded editions, rollback copies, old build directories and caches were removed. They remain in git history: `git checkout bf75693 -- "<path>"`. See [assets/tools/cleanup/info-cleanup-2026-09-25.md](assets/tools/cleanup/info-cleanup-2026-09-25.md), beside the per-file manifest and rules. Going forward, do not copy superseded editions into `info/archive/`. Commit the current state before replacing files, and let git history hold earlier editions. Do not add browser profiles or caches to `info/`; point Chromium `--user-data-dir` at a scratch location or delete the folder after a build.

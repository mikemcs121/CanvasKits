...
The project root holds shared assets, this file, the [canvas-kits.md](canvas-kits.md) index, and one folder per kit. Each kit root contains exactly three production files and one supporting directory:
...
- `info/`: the supplied originals and the sources needed to change the three files and `assets/`, with a short README listing them and the rebuild commands.

By the user's September 25 request, a kit root (and any kit subfolder with its own guide, such as `simplified/`) may also hold `assets/`: lossless copies of the images embedded in that folder's painting guide, produced by `Tools/guide-image-extractor.cjs`. Step panels the guide draws over (brush overlays, the drying desk scene) are instead rendered from the guide at 600 DPI and cropped to the panel, so every step file shows the panel as printed. Do not move these into info/. The project-root `Tools/` folder holds that extractor and `Tools/KitViewer/` (double-click `Kit Viewer.cmd` to browse kits in an Edge/Chrome app window); shared build tools stay in `assets/tools/`.

Each kit's `info/` holds only `archive/previous-root-files/` (supplied originals, kept unchanged) and files that help change the root reference, outline, guide and `assets/`: the reference master PNG, guide step pictures and the sources and builders that make them, the page-colored logo, and simplified-edition builders. Do not keep generated exports, review images, check JSON, manifests, test art or old editions there; rebuild outputs into a scratch location or delete them once the root file is promoted.

September 25, 2026 cleanups (by the user's request): info/ was reduced in two passes, recorded in [pass 1](assets/tools/cleanup/info-cleanup-2026-09-25.md) and [pass 2](assets/tools/cleanup/info-cleanup-2026-09-25-pass2.md). Removed files are in git history (`git checkout ee3dfae -- "<path>"`, or `bf75693` for files removed in pass 1). Do not copy superseded editions into `info/archive/`; commit before replacing files and let git history hold earlier editions. Do not add browser profiles or caches to `info/`. Shared jimp/resvg libraries live in `assets/tools/vector-tools/`. `assets/tools/reference-pdf/png-to-pdf.cjs` writes a reference PDF from its 2400 x 3000 master PNG.

Production files at the kit root are the authoritative customer/transfer deliverables. Do not leave additional aliases, README files, PNGs, or alternate PDFs there. Do not select files by modification date alone. Review the outline, reference, and guide together before promoting them. Preserve file hashes and an old-to-new path manifest when reorganizing.
...
Use [canvas-kits.md](canvas-kits.md) to find current production files and review status. Earlier write-ups (history, organization audits, restoration plan/review/handoff, texture review, template and transfer notes) were retired on September 25 and are in git history (`git show bf75693:"<file>"`). Their paths and superseded standards are historical, not current instructions.

## Artwork and production standards

September 21, 2026 restoration is complete (review and handoff notes are in git history). Four restored masters live in their kits' `info/assets/reference-restoration/restored-master.png`; `assets/tools/guide-template/kits.json` is the only guide plan. Earlier organization/sky-repair exporters are historical and must not overwrite these selected masters. All approved transfer files remain unchanged; preserve them in future work. Existing paint sets and template palettes were retained.

Use the project build-canvas skill for full kits. Its output layout must follow the three-file root structure above. A full kit includes the three production files plus the edit sources described above under info/.
...
Review all three files together, including rendered SVG/reference/guide images. Verify exact SVG size, vector content, gray and 2-point weight; one-page PDF sizes; outline/reference registration; cumulative painting sequence; distinct finishing panels; all recipes and spelling; legible text; logo integrity; and matching final PDF/PNG revisions. Report the checks actually performed; keep check files out of info/ unless a builder reads them. Do not equate correct page dimensions with correct geometry.

Rasterize PDFs with the shared renderer `assets/tools/pdf-render/render-pdf.cjs`, run from the project root:

```
node assets/tools/pdf-render/render-pdf.cjs "<kit>/<slug>-painting-guide-8x10.pdf" "<scratch>/<slug>-painting-guide-8x10.png" --expect-pages 1 --expect-points 612x792 --checks "<scratch>/pdf-checks.json"
```
